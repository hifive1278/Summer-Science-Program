...
#Command line args:  python ApPhot.py 490 293 5 8 13
...
#Constants (need to change for real application)
dark = 10 #e/pix
gain = 0.8 #e/ADU
read = 11 #e/pix
...
#Added for Astro Pset 7, print out total signal of aperture
apSignal = pixApertureCrop.sum()
print("Aperture signal:", apSignal)

# apSignal = 16683

numeratorSNR = math.sqrt(apSignal * gain)
denomSNR = math.sqrt(1+numAperturePix * (1+numAperturePix/numAnnulusPix) * ((meanBackground*gain + dark + read**2 + (gain**2)/12) / (apSignal*gain)))
SNR = numeratorSNR / denomSNR
print("SNR:", SNR)
print("Signal uncertainty:", apSignal / SNR)
print("Instrumental magnitude:", -2.5*math.log10(apSignal))
print("Instrumental magnitude uncertainty:", 1.0875 / SNR)
...
for row in range(1024):
    for column in range(1024):
        if annulusIR < math.sqrt((column - meanXPosition)**2.0 + (row - meanYPosition)**2.0) < annulusOR:
            #Based off middle of pixel
            annulusPixSum += imageArr[row, column]
            numAnnulusPix += 1
meanBackground = annulusPixSum / numAnnulusPix
...
for row in range(1024):
    for column in range(1024):
        if math.sqrt((column - meanXPosition)**2.0 + (row - meanYPosition)**2.0) < apertureRadius:
            numAperturePix += 1 #Make sure number of pixels in range is same as told (69)
            if (imageArr[row,column] - meanBackground) > 0:
                pixAperture[row, column] = imageArr[row, column] - meanBackground
print("\nPixel count:", numAperturePix)
...
for column in range(len(listOfColumns)):
    if (listOfColumns[column] > 0):
        listOfColumns[column] *= (column + (meanXPosition - apertureRadius)) #Element 0 in this list is column 484 in original array (490 - 5 - 1);;; ACTUALLY, it's (489 - 5) because we subtracted one from X and Y at beginning

for row in range(len(listOfRows)):
    if (listOfRows[row] > 0): #Exclude empty rows
        listOfRows[row] *= (row + (meanYPosition - apertureRadius)) #Element 0 in this list is row 287 in original array (293 - 5 - 1); subtract 1 because did so in lines 20-23 when establishing cropped FOV to make it larger

newMeanXPosition = listOfColumns.sum() / unchangedColumnList.sum()
newMeanYPosition = listOfRows.sum() / unchangedRowList.sum()
print("\nNew X coordinate:", newMeanXPosition, "\nNew Y coordinate", newMeanYPosition)

#Find the numerator in the uncertainty formula for x-coordinate
# numeratorX = 0
# currTerm = 0
# for i in range(len(unchangedColumnList)):
#     currTerm = unchangedColumnList[i]*((i+(meanXPosition - apertureRadius) - newMeanXPosition)**2) #Used to be ((i+484 - meanXPosition)**2), changed like in line 56 and 60
#     numeratorX += currTerm
...
# currTerm = 0
# for i in range(len(unchangedRowList)):
#     currTerm = unchangedRowList[i]*(i+(meanYPosition - apertureRadius) - newMeanYPosition)**2 #Used to be ((i+287 - meanYPosition)**2), changed like in line 56 and 60
#     numeratorY += currTerm
